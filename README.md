...
- Code inside `<script type="module">` has own scope. Multiple of them cannot communicate with each other
- module as separate js file uses `import` `export`, and only works in HTTPS
- **module 模块代码仅在第一次导入时被解析** (very similar to C++'s include guard)
